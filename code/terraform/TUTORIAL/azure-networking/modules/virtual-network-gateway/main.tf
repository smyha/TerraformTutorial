# ============================================================================
# Azure Virtual Network Gateway Module - Main Configuration
# ============================================================================
# Virtual Network Gateway provides secure connectivity between:
# - On-premises networks and Azure Virtual Networks (VPN Gateway)
# - On-premises networks and Azure Virtual Networks (ExpressRoute Gateway)
# - Azure Virtual Networks (VNet-to-VNet)
#
# Gateway Types:
...
# ----------------------------------------------------------------------------
# Virtual Network Gateway requires a public IP address for connectivity.
# - VPN Gateway: Requires public IP for Internet connectivity
# - ExpressRoute Gateway: May require public IP for certain configurations
# In active-active mode, two public IPs are required.
# ----------------------------------------------------------------------------
resource "azurerm_public_ip" "main" {
  count = var.active_active ? 2 : 1
  
  name                = "${var.gateway_name}-pip-${count.index + 1}"
...
# ----------------------------------------------------------------------------
# Virtual Network Gateway provides:
...
#
# - ExpressRoute Gateway (type = "ExpressRoute"):
#   * Private connectivity: Dedicated connection via service provider
...
resource "azurerm_virtual_network_gateway" "main" {
  ...
  vpn_type            = var.gateway_type == "Vpn" ? var.vpn_type : null
  sku                 = var.sku
  ...
  dynamic "vpn_client_configuration" {
    for_each = var.gateway_type == "Vpn" && var.vpn_client_configuration != null ? [var.vpn_client_configuration] : []
    content {
